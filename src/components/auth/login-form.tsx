...
import { Lock, Loader2, LogIn, Mail } from 'lucide-react'
...
export function LoginForm() {
	...
	const onSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
		...
		try {
			...
			toast.success('Kirish muvaffaqiyatli.', { richColors: true })
			router.replace('/chat')
			router.refresh()
		} catch (error) {
			const message =
	...
	return (
		<div className='auth-shell'>
			<form onSubmit={onSubmit} className='auth-card auth-card-elevated'>
				...
				<Link href='/register' className='auth-link auth-link-center'>
					{"Ro'yxatdan o'tish"}
				</Link>
			</form>
		</div>
